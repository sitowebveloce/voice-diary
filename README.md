# Voice Diary

![Voice Diary](imgs/voicediary.gif)

A simple voice diary, set the language, click the microphone, you speak, he writes.

### Techno
- HTML
- CSS
- JAVASCRIPT

## Run
To run this project you can simple open the index.html file or use [http-server](https://www.npmjs.com/package/http-server)

- You need to have [Node.js](https://nodejs.org/) installed in your system.
- Open a cmd or terminal prompt, run the command:
```
 npm install http-server -g
```

- Navigate to the specific path of your file folder in CMD or Terminal, and run the command 
```
http-server
```

- Go to your browser and type 
```
localhost:8080
```
Your Application should run there.

## License
[MIT](https://choosealicense.com/licenses/mit/)

author ❤️ @lexpaper


